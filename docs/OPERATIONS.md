...
```yaml
projects:
  - id: prj838              # 英数小文字。state・ログ・封筒で使う識別子
    name: KANAMETO          # 表示名
    ...
    backlog:
      ...
      project_keys: [PRJ838]
      api_key_secret: ws-backlog-key-prj838   # 値は Secret Manager 名のみ
    gws:
      refresh_token_secret: ws-gws-token-prj838
    slack:
      user_token_secret: ws-slack-token-prj838
      team_id: T0XXXXXXX
...
BFF は `--iap` 付きでデプロイされ、`X-Goog-Authenticated-User-Email` ヘッダで本人確認する。利用許可は Google グループに `roles/iap.httpsResourceAccessor` を付与し、日々の利用者追加はグループのメンバー管理のみで行う。
...
| メンバー・アンカー・接続設定 | `config/projects.yaml` | Agent Engine **と** BFF(両方が展開する) |
